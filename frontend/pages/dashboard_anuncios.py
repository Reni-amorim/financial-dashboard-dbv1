...
# Todas as métricas vêm do anuncios_calculator via API
m = data.get("metricas", {})
...
with col1:
    st.metric("📢 Total de Anúncios", m.get("total_anuncios", 0))
with col2:
    ativos = m.get("status_counts", {}).get("Ativo", 0)
    st.metric("✅ Ativos", ativos)
with col3:
    st.metric("👁️ Impressões", f"{m.get('total_impressoes', 0):,}".replace(",", "."))
with col4:
    st.metric("🖱️ Cliques", f"{m.get('total_cliques', 0):,}".replace(",", "."))

col5, col6, col7, col8 = st.columns(4)
with col5:
    st.metric("💰 Receita",      fmt(m.get("total_receita", 0)))
with col6:
    st.metric("💸 Investimento", fmt(m.get("total_investimento", 0)))
with col7:
    st.metric("📈 ROAS", f"{m.get('roas_global', 0):.2f}x",
              help="Receitas / Investimento")
with col8:
    st.metric("📉 ACOS", f"{m.get('acos_global', 0):.2f}%",
              help="Investimento / Receitas")
...
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric("CTR médio", f"{m.get('ctr_medio', 0):.2f}%",
              help="Click Through Rate — média dos anúncios com cliques")
with col2:
    st.metric("CPC médio", f"R$ {m.get('cpc_medio', 0):.2f}",
              help="Custo por Clique")
with col3:
    st.metric("CVR médio", f"{m.get('cvr_medio', 0):.2f}%",
              help="Conversion Rate")
with col4:
    vendas = m.get("total_vendas", 0)
    vdir   = m.get("total_vendas_diretas", 0)
    vind   = m.get("total_vendas_indiretas", 0)
    st.metric("🛒 Vendas Totais", vendas,
              help=f"{vdir} diretas + {vind} indiretas")
...
# ─────────────────────────────────────────────────────────────
por_campanha = m.get("por_campanha", [])
if por_campanha:
    ...
    with tab1:
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df_camp["campanha"], y=df_camp["receita"],
                             name="Receita", marker_color="#2ecc71"))
        fig.add_trace(go.Bar(x=df_camp["campanha"], y=df_camp["investimento"],
                             name="Investimento", marker_color="#e74c3c"))
        fig.update_layout(barmode="group", xaxis_title="Campanha",
    ...
    with tab2:
        fig2 = px.bar(
            ...
            x="roas", y="campanha", orientation="h",
...
# TOP 10 ANÚNCIOS
# ─────────────────────────────────────────────────────────────
top_anuncios = m.get("top_anuncios", [])
if top_anuncios:
    ...
    df_display["receita"]      = df_display["receita"].apply(fmt)
    df_display["investimento"] = df_display["investimento"].apply(fmt)
